...
class HiddenMarkovModel(object):
    ...
    def predict_one_step_ahead(self, current_state_probs, observation_map=None):
        next_state_probs = torch.matmul(current_state_probs.unsqueeze(0), self.T).squeeze(0)
        next_obs_probs = torch.matmul(next_state_probs.unsqueeze(0), self.E.T).squeeze(0)
    ...
    def evaluate(self, observations, mode='classification', actual_values=None, actual_labels=None, observation_map=None):
        ...
        if mode == 'classification':
            ...
            for state in unique_states:
                ...
                state_to_label[state] = 1 if avg_label >= 0.5 else 0
            ...
            metrics['state_interpretations'] = self.interpret_states(states_seq, actual_values, actual_labels)
            ...
            forecasts = []
            actual_next_values = actual_values[1:] if actual_values is not None else None
            
            for t in range(len(observations) - 1):
                current_probs = torch.zeros(self.S, dtype=torch.float64)
                current_probs[states_seq[t]] = 1.0
                
                _, prediction = self.predict_one_step_ahead(current_probs, observation_map)
